...
var db = window.openDatabase('mydb', '1.0', 'Test DB', 10 * 1024 * 1024); 
 ...
 //'noviembre VARCHAR(100), diciembre VARCHAR(100))'); 
  //tx.executeSql('INSERT INTO televent VALUES (1, "fbfffar", 3.5, 6.4, "", "", "", "", "", "", "", "", "", "", "", "")'); 
    ...
    ' estado VARCHAR(15) NOT NULL, comentarios TEXT(250) NOT NULL, FOREIGN KEY (codigo_mantenimiento) REFERENCES mantenimientos (codigo_mantenimiento))'); 
    tx.executeSql('INSERT INTO pedido_mantenimiento VALUES (1, "qfdddfq", "7.5", "2.7", "345", "cc", "ccc")'); 
    ...
    ' numero_operarios INT(11) NOT NULL, tiempo_mantenimiento TIME NOT NULL,'+
    ' cantidad INT(11) NOT NULL, precio_unidad DECIMAL(15.2) NOT NULL, codigo_operarios INT(11) NOT NULL,'+
    ' FOREIGN KEY (codigo_pedido) REFERENCES pedido_mantenimiento (codigo_pedido), FOREIGN KEY (codigo_operarios) REFERENCES operarios_mantenimiento (codigo_operarios))'); 
     tx.executeSql('INSERT INTO detalle_mantenimiento VALUES (1, 7.9, 6.7, 2, 2.5, 24)'); 
    ...
    tx.executeSql('INSERT INTO producto_mantenimiento VALUES (1, "qC", "ngs", 2.7, "dd", "ddff", "sss", "qq", "33")'); 
 
    tx.executeSql('DROP TABLE IF EXISTS operarios_mantenimiento');
    tx.executeSql('CREATE TABLE IF NOT EXISTS operarios_mantenimiento (codigo_operarios INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,'+
    ' operario_1 VARCHAR(45) NOT NULL, operario_2 VARCHAR(45) NOT NULL, operario_3 VARCHAR(45) NOT NULL, operario_4 VARCHAR(45) NOT NULL, operario_5 VARCHAR(45) NOT NULL)'); 
    tx.executeSql('INSERT INTO operarios_mantenimiento VALUES (1, "qqffq", 7.9, 2.7, "ss", "ee")'); 
 

    tx.executeSql('DROP TABLE IF EXISTS gama_producto');
    tx.executeSql('CREATE TABLE IF NOT EXISTS gama_producto (gama VARCHAR(15) PRIMARY KEY,'+
    ' descripcion_texto VARCHAR(100) NOT NULL, descripcion_html FLOAT NOT NULL, imagen FLOAT NOT NULL)'); 
    tx.executeSql('INSERT INTO gama_producto VALUES (1, "ddd", 7.9, 2.7)'); 
...
// 
// 
let menu = document.querySelector("#tab-bar");
console.log("aadddddddddddddddddddddddddddddddddddddddddddddd " );
console.log("childElementCount "  + menu.childElementCount);
console.log("aadddddddddddddddddddddddddddddddddddddddddddddd " );
...
for (let recorreMenu = 0; recorreMenu<menu.childElementCount;recorreMenu++){ 
        console.log("menu " + recorreMenu);
         menu.children[recorreMenu].addEventListener('click', function(e) {
            console.log("menu " + recorreMenu);
            if (recorreMenu===0){
                // map.classList.remove('current'); 
                camera.classList.remove('current'); 
                twitter.classList.remove('current'); 
                map.classList.add('current');

            }else if  (recorreMenu===1){
                map.classList.remove('current'); 
                // camera.classList.remove('current'); 
                twitter.classList.remove('current'); 
                camera.classList.add('current');
            }else if  (recorreMenu===2){
                map.classList.remove('current'); 
                camera.classList.remove('current'); 
                // twitter.classList.remove('current'); 
                twitter.classList.add('current');}
          });

    }
// aa.addEventListener("click", 
//         function() {
//             console.log("ssdfg  fgdf  gfd fgfdgf dgfds " );
